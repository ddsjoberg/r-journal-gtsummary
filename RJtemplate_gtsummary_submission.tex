...
% TO DO
% - change all code refs from \code{} to \code{}
% - update all references to packages to use \CRANpkg{} and \pkg{}
% - scale the images appropriately (they are too small now)
...
The \code{tbl\_regression()} function leverages the huge effort behind the broom package's \code{tidy()} function \citep{broom} to perform the initial formatting of the regression object.
Because \code{tbl\_regression()} utilizes \code{broom::tidy()}, there are many model types that are supported out of the box, such as \code{lm()}, \code{glm()}, \code{lme4::lmer()}, \code{lme4::glmer()}, \code{geepack::geeglm()}, \code{survival::coxph()}, \code{survival::survreg()}, \code{survival::clogit()}, \code{nnet::multinom()}, \code{rstanarm::stan\_glm()}, models built with the mice package[add ref]and more. The \pkg{gtsummary} package also contains built in tidiers to report standardized coefficients and bootstrapped confidence intervals, as well as the ability to provide your own custom tidiers.
...
A simple call to the \code{inline\_text()} function will dynamically add the model results to a an Rmarkdown report.
...
The \pkg{gtsummary} tables shown above are often ready for publication as they are, however it is common that higher quality tables can be constructed by merging or stacking simple tables.
...
These two models are often reported side-by-side, and it's simple to construct this table using  the \code{tbl\_merge()} function.
First, build a table for each regression model using \code{tbl\_regression()}, then merge the two tables with \code{tbl\_merge()}. Any number of \pkg{gtsummary} tables can be merged with this function.
...
Similarly, any number of \pkg{gtsummary} tables may be stacked on top of one another using the \code{tbl\_stack()} function.
...
Several themes are available in the package that follow various journals' reporting guidelines, reduce cell padding, and recently language themes were introduced and the output of \pkg{gtsummary} tables can be translated into more than 10 languages.
...
A custom theme was used to construct the \pkg{gtsummary} tables shown in this manuscript to match the \emph{R Journal} font. 
...
The package was written to be a companion to the \pkg{gt} package from RStudio and is optimized to leverage the advanced customization features of this print engine, but offers compatibility with a variety of popular printing methods including \code{knitr::kable()}, \pkg{flextable}, \pkg{huxtable} and more.
...
The package is designed to interact with these print engines behind the scenes to reduce burden on users, and you generally only need to be aware of them if you want to add advanced customizations (eg. example X with \code{tab\_header}). The default print engine can be systematically set using \pkg{gtsummary} themes.
