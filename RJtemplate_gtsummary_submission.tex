...
Identifying building blocks of tables presented in medical literature (that also are common among all fields), \pkg{gtsummary} seeks to create these fully-formatted tables so there is no need for further end-user modifications.  
...
For example, users can take advantage of existing \CRANpkg{broom} tidiers to clean regression results for \texttt{tbl\_regression()} and use \CRANpkg{gt}, \CRANpkg{kable}, \CRANpkg{flextable} or others to print \pkg{gtsummary} tables to various output formats (e.g. HTML, PDF, Word, or RTF).
Furthermore, all \pkg{gtsummary} functions are created to work within a “tidy” framework where functions can be used with pipes.
...
\pkg{gtsummary} uses \CRANpkg{glue} to help interpret string literals. Glue embeds R expressions (functions or objects) in curly braces which are then evaluated and inserted into the argument string. 
...
In the example above it could have been used to change the summary statistics for all continuous variables---\code{all\_continuous() ~ c("\{N\_nonmiss\}", "\{mean\} (\{sd\})")}. 
...
In addition to basic summary statistics, the \pkg{gtsummary} package has several functions to add additional information or statistics to \texttt{tbl\_summary()} tables.
...
The \pkg{gtsummary} has two functions to assist researchers and analysts prepare tabular summaries of regression models: \texttt{tbl\_regression()} and \texttt{tbl\_uvregression()}.
...
The\texttt{inline\_text()}  function reports statistics from \pkg{gtsummary} tables inline in an R markdown document.
...
First, build a table for each regression model using \texttt{tbl\_regression()}, then merge the two tables with \texttt{tbl\_merge()}. Any number of \pkg{gtsummary} tables can be merged with this function.
...
Similarly, any number of \pkg{gtsummary} tables may be stacked on top of one another using the \texttt{tbl\_stack()} function.
...
The package was written to be a companion to the \pkg{gt} package from RStudio and is optimized to leverage the advanced customization features of this print engine, but offers compatibility with a variety of popular printing methods including \texttt{knitr::kable()}, \pkg{gtsummary}, \pkg{huxtable} and more.
While \pkg{gt} is used as default for most outputs, you can easily convert to using print engine of choice using several conversion helper functions provided in the package (e.g. \texttt{as\_kable()}).
