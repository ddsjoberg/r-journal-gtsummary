...
Furthermore, \CRANpkg{tableone} reports on general descriptive statistics, it is fundamentally lacking in its ability to produce tables for regression modeling strategies in R. 
...
The \pkg{gtsummary} tables shown above are often ready for publication as they are.
...
Simply, build a table for each regression model using \texttt{tbl\_regression()}, then merge the to tables with \texttt{tbl\_merge()}: any number of \pkg{gtsummary} tables can be merged with this function.
