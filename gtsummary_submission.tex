...
After identifying these basic structures of most tables presented in medical literature (and other fields), we wrote \pkg{gtsummary} to ease creation of fully-formatted, ready to publish tables.
...
Along with descriptive summaries, \pkg{gtsummary} summarizes statistical models, survey data, and builds cross tabulations. 
...
All tables in this manuscript were created using \pkg{gtsummary} v1.4.0.
...
Additionally, \code{NA} values are recognized as missing and listed as unknown, and if a data set is labelled, the label attributes are utilized. 
...
For example, using the theme for \textit{The Journal of the American Medical Association} (\textit{JAMA}), large p-values are rounded to two decimal places, confidence intervals are shown as \code{"lb to ub"} instead of \code{"lb, ub"}, and the confidence internal is displayed in the same column as the model coefficients. 
